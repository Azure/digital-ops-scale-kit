...
- **Resource sets**: each ordered list under `properties.resourceSets` names reusable resource definitions from the matching `parameters/<area>/` directory. Omit an area or use `[]` for no selection. Deselecting a set stops applying it and does not delete resources. See `docs/resource-catalog.md`.
