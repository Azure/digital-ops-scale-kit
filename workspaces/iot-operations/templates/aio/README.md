...
| **Per-API-version modules** | `modules/` | Inner modules a dispatcher routes to, one per API generation. Added only where an API version genuinely diverges. |
| **Resource catalog families** | `dataflows/` | A `main.bicep` routing on the AIO API version to one module per generation under the family's own `modules/`. Deployed through `manifests/aio-resources.yaml`, gated per site. |
...
computed, which is why an API generation that diverges needs its own module
...
supported AIO API generation, and each module creates every kind the family owns,
...
A family writes at the generation the site's release ships, matching the platform
templates. The writable surface of these resources is identical across supported
generations today, but schema equality does not imply the provider behaves the
same way, so the generation is not assumed to be interchangeable. Adding a
generation means extending `@allowed` on `main.bicep` and copying the newest
...
- A new API generation that diverges goes in `modules/`, reached by a dispatcher.
