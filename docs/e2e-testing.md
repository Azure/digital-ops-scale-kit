...
|-------|--------------|-------|
| `aio-releases` | `2606` or `2605,2606` | Comma-separated. Ephemeral fans out in parallel. Persistent serializes cells in the same RG. See [aio-releases.md](aio-releases.md) for how releases are defined and pinned. |
...
| `upgrade-to` | empty or `2606` | Optional AIO release to upgrade to after install-phase tests pass. Empty skips the upgrade phase. Per-cell skip when equal to the cell's `aio-releases` value. Requires `aio-upgrade` to be in the `tests` allowlist (or `tests` empty). |
